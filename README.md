...
**Pulsanti nei messaggi di allerta:** Quando è presente un'allerta meteo (oggi o domani), i messaggi di allerta includono un inline keyboard su due righe: `[🗺️ Mappe allerta] [🌤️ Meteo]` (→ rispettivamente `https://www.regione.toscana.it/allertameteo` e `https://www.lamma.toscana.it/meteo/bollettini-meteo/toscana`) + `[📋 Cosa fare]` (→ `https://www.regione.toscana.it/allertameteo/rischi-e-norme-di-comportamento`). Il messaggio con previsioni complete del broadcast usa la stessa keyboard delle allerte. Il pulsante "🖼️ Previsioni complete" (→ `https://www.lamma.toscana.it/meteo/bollettini-meteo/toscana`) resta disponibile nel comando `/previsioni`.
